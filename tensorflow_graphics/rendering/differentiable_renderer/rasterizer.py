...
def rasterize(vertices,
  ...
  barycentric coordinate maps. The image size is determined by `image_width` and
  `image_height`. The maximum value of depth map is set to be `max_depth`. The
  triangle index map and barycentric coordinate map will be -1 for empty pixels.
  `min_depth` determines the depth of the near clipping plane. When optimizing
  with a loss function defined over a depth map, `min_depth` can be set to a
  negative value to prevent negative translation hypotheses from getting stuck.
